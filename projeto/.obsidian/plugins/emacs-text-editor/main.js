...
var WORD_CHAR_REGEX = /\w/u;
var KEY_CODE_KEY_REGEX = /^Key[A-Z]$/u;
var KEY_CODE_DIGIT_REGEX = /^Digit[0-9]$/u;
var KEY_CODE_NUMPAD_REGEX = /^Numpad[0-9]$/u;
var WORD_BOUNDARY_REGEX = /\b/gu;
...
var EmacsTextEditorPlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      editorCallback: (editor, _) => {
        this.withSelectionUpdateDirect(
          editor,
  ...
  moveForwardOneWord(editor) {
    this.withSelectionUpdateDirect(editor, (head) => {
      ...
      while (offset < len && !WORD_CHAR_REGEX.test(doc[offset]))
        offset++;
      while (offset < len && WORD_CHAR_REGEX.test(doc[offset]))
        offset++;
  ...
  moveBackOneWord(editor) {
    this.withSelectionUpdateDirect(editor, (head) => {
      const doc = editor.getValue();
      let offset = editor.posToOffset(head);
      while (offset > 0 && !WORD_CHAR_REGEX.test(doc[offset - 1]))
        offset--;
      while (offset > 0 && WORD_CHAR_REGEX.test(doc[offset - 1]))
        offset--;
      return editor.offsetToPos(offset);
  ...
  withSelectionUpdate(editor, callback, directMove) {
  ...
  withSelectionUpdateDirect(editor, directMove) {
    if (this.disableSelectionWhenPossible) {
      this.disableSelection(editor);
    }
    const currentSelectionStart = this.getCurrentSelectionStart(editor);
...
function isEventInterruptSelection(e) {
  ...
  return e.code == "Backspace" || e.code == "Delete" || KEY_CODE_KEY_REGEX.test(e.code) && !withKeyModifier || KEY_CODE_DIGIT_REGEX.test(e.code) && !withKeyModifier || KEY_CODE_NUMPAD_REGEX.test(e.code) && !withKeyModifier || insertableSpecialKeys.includes(e.code) && !withKeyModifier;
}
